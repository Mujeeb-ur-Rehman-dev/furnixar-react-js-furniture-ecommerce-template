...
export default function ShopV1() {
    ...
    const [visibleProducts, setVisibleProducts] = useState(12); // Initial 12 products (4 columns × 3 rows)
    const itemsPerPage = 12; // Load 12 more products each time
        ...
        <div className="s-py-100">
            <div className="container-fluid">
                <div className="flex items-start justify-between gap-8 max-w-[1720px] mx-auto flex-col lg:flex-row border-b border-bdr-clr dark:border-bdr-clr-drk pb-8 md:pb-[50px]" data-aos="fade-up" data-aos-delay="100">
                    <div>
                        ...
                        <div className="flex flex-wrap gap-[10px] md:gap-[15px]">
                            <Link className="btn btn-theme-outline btn-sm shop1-button" to="/product-category/console-table" data-text="Console Table"><span>Console Table</span></Link>
                            <Link className="btn btn-theme-outline btn-sm shop1-button" to="/product-category/nesting-tables" data-text="Nesting Tables"><span>Nesting Tables</span></Link>
                            <Link className="btn btn-theme-outline btn-sm shop1-button" to="/product-category/side-tables" data-text="Side Tables"><span>Side Tables</span></Link>
                            <Link className="btn btn-theme-outline btn-sm shop1-button" to="/product-category/coffee-table" data-text="Coffee Table"><span>Coffee Table</span></Link>
                            <Link className="btn btn-theme-outline btn-sm shop1-button" to="/product-category/office-desk" data-text="Office Desk"><span>Office Desk</span></Link>
                            <Link className="btn btn-theme-outline btn-sm shop1-button" to="/product-category/rack" data-text="Rack"><span>Rack</span></Link>
                            <Link className="btn btn-theme-outline btn-sm shop1-button" to="/product-category/wardrobe" data-text="Wardrobe"><span>Wardrobe</span></Link>
                        </div>
                    </div>
                ...
                <div className="max-w-[1720px] mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 sm:gap-8 pt-8 md:pt-[50px]" data-aos="fade-up" data-aos-delay="300">
                    {productList.slice(0, visibleProducts).map((item,index)=>{
                        return(
                ...
                {visibleProducts < productList.length && (
                    <div className="text-center mt-7 md:mt-12">
                        <button 
                            onClick={() => setVisibleProducts(prev => Math.min(prev + itemsPerPage, productList.length))}
